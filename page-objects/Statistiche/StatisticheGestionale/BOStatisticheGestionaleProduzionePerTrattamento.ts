...
export default class BOStatisticheGestionaleProduzioneOccupazionePerTrattamento{
    ...
    constructor(public page: Page, container?) {
        ...
        this.table = this.container.locator('table.tbldata.dataTable').first();
    ...
    async checkFilterPanel() {
      ...
      //  await expect(this.refDataSelect).toBeVisible();
     //   await expect(this.filterBySelect).toBeVisible();
        await expect(this.searchButton).toBeVisible();
    }

    async checkSearchInviteMessage() {
        await expect(this.searchInviteMessageContainer).toBeVisible();
        /*   const message = await this.searchInviteMessageContainer.textContent();
           const trimmedMessage = message ? message.trim() : '';
           console.log(`Contenuto del messaggio di invito: "${trimmedMessage}"`);*/
        await expect(this.searchInviteMessageContainer).not.toBeEmpty();
    }
}